...
export async function suggestCharacters(
  ...
  const prompt = `
你是一位专业的漫画设定集编剧。请深入分析以下【故事背景】，并提取或建议 2-3 个核心角色。
...
**任务目标：**
1. **优先提取：** 如果背景描述中已经出现了具体的人名（例如：“少年阿强”、“博士 Z”），请务必将其作为首选建议角色提取出来。
2. **细节捕捉：** 仔细观察背景中提到的特定物品（如“生锈的长剑”）、装备（如“发光的护目镜”）或生理特征（如“蓝色的皮肤”），并将这些元素融入到角色的外观描述中。
3. **逻辑补完：** 如果背景中没有明确提及人名，请根据世界观设定（如赛博朋克、仙侠、废土等）建议最能体现该背景特征的角色。

**角色输出要求：**
- **name:** 角色名字（优先使用背景中的名字）。
- **description:** 详细的外观描述。必须包含背景中提到的相关特征，并补充年龄、发型、服装风格及性格表现，使其具有强烈的视觉画面感。

请严格以 JSON 数组格式返回：
[
  {"name": "人名", "description": "基于背景细节的视觉描述..."},
  ...
]
  `.trim();
